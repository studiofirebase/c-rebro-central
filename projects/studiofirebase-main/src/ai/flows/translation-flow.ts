...
const TranslateInputSchema = z.object({
  ...
  maxLength: z.number().int().positive().optional().describe('Limite maximo de caracteres para o texto traduzido.'),
});
...
const translateFlow = ai.defineFlow(
  ...
  async (input: TranslateInput) => {
    const { text, targetLanguage, maxLength } = input;
    const lengthHint = maxLength ? `Keep it within ${maxLength} characters.` : 'Keep it concise.';
    const prompt = [
