/**
 * Instagram Chat Adapter
 *
 * Gerencia mensagens do Instagram Direct usando o storage no Firestore.
 */
...
export class InstagramAdapter extends BaseChatAdapter {
  readonly channel = CHAT_CHANNELS.INSTAGRAM;

  private getMessagesCollection(adminUid: string) {
    return getChannelMessagesCollection(adminUid, CHAT_CHANNELS.INSTAGRAM);
  }
  ...
  private async getIntegrationData(adminUid: string): Promise<InstagramIntegrationData | null> {
    const adminApp = getAdminApp();
    if (!adminApp) return null;

    const db = getDatabase(adminApp);
    const perAdminSnap = await db.ref(`admin/integrations/${adminUid}/instagram`).get();
    const perAdmin = perAdminSnap?.val();

    if (perAdmin && typeof perAdmin === 'object') {
      return perAdmin as InstagramIntegrationData;
    }

    const globalSnap = await db.ref('admin/integrations/instagram').get();
    const globalData = globalSnap?.val();
    if (globalData && typeof globalData === 'object') {
      return globalData as InstagramIntegrationData;
    }
  ...
  async fetchConversations(adminUid: string): Promise<UnifiedConversation[]> {
    try {
      ...
      for (const [participantId, msgs] of conversationMap.entries()) {
        ...
        conversations.push({
          id: participantId,
          channel: CHAT_CHANNELS.INSTAGRAM,
          participant: {
            id: participantId,
            name: participantId,
      ...
      console.error('[InstagramAdapter] Erro ao buscar conversas:', error);
      return [];
    }
  }
  ...
  async fetchMessages(conversationId: string, adminUid: string, limit: number = 100): Promise<UnifiedMessage[]> {
    try {
      ...
      console.error('[InstagramAdapter] Erro ao buscar mensagens:', error);
      throw error;
    }
  }
    ...
    try {
      ...
      if (!accessToken) {
        return this.createErrorResult(
          ...
          'Access token do Instagram não encontrado'
        );
      }
      ...
      const response = await fetch(`https://graph.facebook.com/${INSTAGRAM_API_VERSION}/${igUserId}/messages`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        return this.createErrorResult(
          ...
          data?.error?.message || 'Falha ao enviar mensagem no Instagram',
          data
        );
      }
