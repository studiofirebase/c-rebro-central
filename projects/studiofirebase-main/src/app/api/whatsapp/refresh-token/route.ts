...
export async function POST(req: NextRequest) {
  try {
    // WhatsApp long-lived token refresh (exemplo fictício, ajuste conforme API real)
    ...
    const refreshUrl = `https://graph.facebook.com/v18.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${process.env.WHATSAPP_APP_ID}&client_secret=${clientSecret}&fb_exchange_token=${accessToken}`;
