...
async function getAccessToken(adminUid: string) {
  ...
  const snap = await db.ref(`admin/integrations/${adminUid}/facebook`).get();
...
export async function POST(request: NextRequest) {
  ...
  const { participantId, message, accessToken, pageId, adminUid } = body || {};
  ...
  if (!token) {
    return NextResponse.json({ success: false, message: 'Access token do Facebook não encontrado.' }, { status: 500 });
  }

  const targetPageId = pageId || participantId;

  const response = await fetch(`https://graph.facebook.com/v19.0/${targetPageId}/messages`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    return NextResponse.json({ success: false, message: data?.error?.message || 'Falha ao enviar Messenger', data }, { status: 500 });
  }
